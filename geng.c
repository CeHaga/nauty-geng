...
static int distance_check(graph *g, int n, int threshold){
        ...
        for (ixx = 0; ixx < xlim; ++ixx)
        {
                ...
                && (dlow & ~x) == 0)
            {
                    ...
                    && accept1b(g,n,x,gx,degx,&rigidx,makeh))
                {
                    ...
                    haschild = TRUE;
#endif
                    spaextend(gx,nx,degx,ne+xc,rigidx,xlbx,xubx,makeh,threshold);
                }
            }
        }
    ...
    if (nx == maxn)
        ...
        for (i = imin; i < imax; ++i)
        {
            ...
            if (accept1(g,n,x,gx,degx,&rigidx))
            {
                ...
                haschild = TRUE;
#endif
                genextend(gx,nx,degx,ne+xc,rigidx,xlbx,xubx,threshold);
            }
        }
...
{
    char *arg;
    boolean badargs,gote,gotmr,gotf,gotd,gotD,gotx,gotX,gotT;
    ...
    for (j = 1; !badargs && j < argc; ++j)
    {
        ...
        if (arg[0] == '-' && arg[1] != '\0')
        {
            ...
            while (*arg != '\0')
            {
                ...
                else SWINT('T',gotT,threshold,"geng -T")
            ...
            if (bipartite)
                if (squarefree)
                    spaextend(g,1,deg,0,TRUE,
                                    data[1].xlb,data[1].xub,makeb6graph,threshold);
                else
                    spaextend(g,1,deg,0,TRUE,
                                    data[1].xlb,data[1].xub,makebgraph,threshold);
            else if (trianglefree)
                if (squarefree)
                    spaextend(g,1,deg,0,TRUE,
                                    data[1].xlb,data[1].xub,makeg5graph,threshold);
                else
                    spaextend(g,1,deg,0,TRUE,
                                    data[1].xlb,data[1].xub,makexgraph,threshold);
            else if (squarefree)
                spaextend(g,1,deg,0,TRUE,
                                    data[1].xlb,data[1].xub,makesgraph,threshold);
            else if (savemem)
                spaextend(g,1,deg,0,TRUE,
                                    data[1].xlb,data[1].xub,make0graph,threshold);
            else
                genextend(g,1,deg,0,TRUE,data[1].xlb,data[1].xub,threshold);
        }
    }
    t2 = CPUTIME;
